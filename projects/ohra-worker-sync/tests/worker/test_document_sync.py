...
@pytest.mark.asyncio
async def test_document_sync_confluence():
    """Confluence 문서 동기화 테스트"""
    test_start = datetime.now()

    test_info = {
        "test_name": "Worker 문서 동기화 테스트 - Confluence",
        "test_type": "worker",
    ...
    print_test_header(
        test_info["test_name"], "Confluence 문서 수집이 정상적으로 동작하는지 확인합니다.", is_evaluation_target=False
    )

    # TODO: 실제 워커 실행 및 문서 수집 테스트
    ...
    save_test_results("worker_document_sync_confluence", test_info, output_dir)
