    ...
    async with aiohttp.ClientSession() as session:
        for i, pair in enumerate(test_pairs, 1):
            ...
            if embedding1 and embedding2:
                similarity = cosine_similarity(embedding1.get("embedding", []), embedding2.get("embedding", []))
                ...
                print(
                    f"  결과: 유사도={similarity:.4f}, 예상={'유사' if expected else '다름'} ({'✅ PASS' if passed else '❌ FAIL'})"
                )
            else:
                results.append(
                    {"text1": pair["text1"], "text2": pair["text2"], "error": "임베딩 생성 실패", "passed": False}
                )
                print("  결과: ERROR - 임베딩 생성 실패")
