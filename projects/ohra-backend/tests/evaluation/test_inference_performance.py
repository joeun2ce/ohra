...
async def test_inference_performance_basic():
    ...
    print(f"[INFO] 목표: 평균 LLM 추론 시간 < 2초\n")
...
async def test_input_token_limit():
    ...
    print(f"[INFO] 목표: 최대 입력 토큰 수 확인 및 성능 분석\n")
    ...
    async with aiohttp.ClientSession() as session:
        for token_count in token_counts:
            ...
            print(f"  결과: 실제 토큰={actual_tokens}, 추론 시간={inference_time:.3f}s, 속도={result['tokens_per_second']:.1f} tokens/s")
...
async def test_output_token_limit():
    ...
    async with aiohttp.ClientSession() as session:
        for max_tokens in max_tokens_list:
            ...
            print(f"  결과: 실제 토큰={actual_tokens}, 생성 시간={inference_time:.3f}s, 속도={result['tokens_per_second']:.1f} tokens/s")
