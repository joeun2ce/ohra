...
async def test_stability_basic():
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [make_chat_request(session, query) for _ in range(concurrent_count)]
    ...
    elapsed_times = [r.get("elapsed_time", 0) for r in responses if isinstance(r, dict) and r.get("status") == 200]
    ...
    test_info["result"] = {
        ...
        "suitable": suitable,
        "suitability_reason": (
            "모든 요청이 성공하고 응답 시간이 목표 이내" if suitable else "성공률 또는 응답 시간이 목표를 초과"
        ),
    }
...
async def test_stability_limit():
    ...
    async with aiohttp.ClientSession() as session:
        for count in concurrent_counts:
            ...
            start_time = time.time()
            tasks = [make_chat_request(session, query) for _ in range(count)]
            responses = await asyncio.gather(*tasks, return_exceptions=True)
            elapsed = time.time() - start_time
            ...
            elapsed_times = [
                r.get("elapsed_time", 0) for r in responses if isinstance(r, dict) and r.get("status") == 200
            ]
            avg_response_time = sum(elapsed_times) / len(elapsed_times) if elapsed_times else 0
            ...
            print(
                f"  결과: 성공={success_count}/{count} ({success_rate:.1f}%), 평균 응답 시간={avg_response_time:.3f}s"
            )

            # 실패율이 50% 이상이면 중단
            if success_rate < 50:
                print("  [WARNING] 성공률이 50% 미만이므로 테스트 중단")
                break
