...
async def test_search_performance_basic():
    ...
    print("[INFO] 목표: Top-K ≥ 3, avg score ≥ 0.7\n")
    ...
    test_info["result"] = {
        ...
        "suitability_reason": (
            "모든 쿼리가 정상 처리됨 (실제 검색 점수는 백엔드 로그 분석 필요)" if all_success else "일부 쿼리 실패"
        ),
    }

    test_info["completed_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    test_info["total_duration"] = time.time() - test_start
    test_info["details"] = {"total_queries": len(QUERY_SET), "success_count": success_count, "results": results}
