...
async def test_context_retention():
    ...
    test_info = {
        ...
        "target": {
            "description": "동일 conversation_id로 2회 요청 → 맥락 반영 확인",
            "expected_value": "후속 질문에서 이전 대화 맥락 반영",
            "threshold": "맥락 반영 확인",
        },
    }
    ...
    print_test_header(
        test_info["test_name"],
        "평가대상: conversation_id 기반으로 후속 질문에서 이전 대화 맥락이 유지되는지 확인합니다.",
        is_evaluation_target=True,
    )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        response1_text = response1.get("response_text", "")
        print(f"  응답: {response1_text[:100]}...")

        await asyncio.sleep(1)

        # 두 번째 요청 (맥락이 필요한 질문)
        print("\n[TESTING] 두 번째 요청 전송 (맥락 필요)...")
        ...
        print(f"  응답: {response2_text[:100]}...")

        # 맥락 반영 여부 확인
        # "그럼"이라는 단어가 있으면 맥락을 참조하는 것으로 간주
        # 또는 첫 번째 응답의 키워드가 두 번째 응답에 포함되는지 확인
        context_keywords = ["아메바", "AI", "질문", "키워드"]
        context_reflected = any(keyword in response2_text for keyword in context_keywords)

        test_info["result"] = {
            "actual_value": f"맥락 반영: {'예' if context_reflected else '부분적'}",
            "achieved": True,
            "suitable": context_reflected,
            "suitability_reason": (
                "후속 질문에서 이전 대화 맥락이 반영됨" if context_reflected else "맥락 반영이 불충분함"
            ),
        }
        ...
        test_info["details"] = {
            ...
            "context_keywords_found": [kw for kw in context_keywords if kw in response2_text],
        }
    ...
    save_test_results("context_retention", test_info, output_dir)
